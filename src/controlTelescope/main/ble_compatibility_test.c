...
static const uint16_t CHAR_4_CUSTOM                = 0xFF04;
...
static const uint8_t char4_name[]  = "Char_4_Custom";
...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
   ...
   /* Characteristic Declaration */
    ...
    /* Characteristic Value */
    [IDX_CHAR_VAL_CUSTOM] =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_4_CUSTOM, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE | ESP_GATT_PERM_READ_ENC_MITM,
      SHORT_CHAR_VAL_LEN, sizeof(char_value), (uint8_t *)char_value}},
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
            ...
            if(gatt_db_handle_table[IDX_CHAR_VAL_CUSTOM] == param->read.handle) {
                ESP_LOGE(EXAMPLE_TAG, "(?) ***** read char4 ***** \n");
            }
       	    break;
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if(gatt_db_handle_table[IDX_CHAR_VAL_CUSTOM] == param->write.handle && param->write.len == 2) {
                    uint8_t write_data[2] = {0x11, 0x22};
                    if(memcmp(write_data, param->write.value, param->write.len) == 0) {
                        if(ledState==0){
                          ledState=1;
...
void app_main(void)
{
  ...
  gpio_set_direction(BLINK_GPIO, GPIO_MODE_OUTPUT);
    ...
    // Initialize NVS (non-volatile storage library). To save key-value pairs in flash memory
    ret = nvs_flash_init();
    ...
    // Initialize the Bluedroid stack (include common definitions)
    esp_bluedroid_init();
    // Enable Bluedroid
    esp_bluedroid_enable();
    // Set the GATT handler (manage communication when connection is established)
    esp_ble_gatts_register_callback(gatts_event_handler);
    // Set the GAP handler (manage connections)
    esp_ble_gap_register_callback(gap_event_handler);
    // Register a profile
    esp_ble_gatts_app_register(ESP_APP_ID);
    // Set the MTU as 33 bytes
    esp_ble_gatt_set_local_mtu(33);
